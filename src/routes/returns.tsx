...
function ReturnsPage() {
  ...
  const orders = MOCK_ORDERS.filter(o => Boolean(o.returns) || o.status === "Replacement" || o.status === "Exchange");
    ...
    <div className="flex min-h-screen w-full bg-background text-foreground">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="mx-auto w-full max-w-[1780px] flex-1 space-y-4 p-4 md:space-y-5 md:p-6">
          ...
          <ReturnsTable orders={orders} loading={loading} onViewOrder={goToOrder} />
        </main>
      </div>
    </div>
  );
